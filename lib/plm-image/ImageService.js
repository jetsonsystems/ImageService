...
var Images = (function() { 
  ...
  function index(options, callback) {
    ...
    if (_.has(options, 'ids')) {
      var imgOut = [];
      async.forEachLimit(options.ids,
                         1,
                         function (oid, next){
                           Images.show(oid,
                                       null,
    ...
    else if (_.isEmpty(options) || options.created || ((options.trashState === 'out') && (!options.filter)) || (options.filter && !_.has(options.filter, 'rules') && _.has(options.filter, 'data') && (_.size(options.filter.data) === 0))) {
